...
export default function InformationPage() {
	const events: EventCard[] = [
		...
		},
		{
			...
			date: "⁠29th August morning to afternoon",
			location: "TU Munich",
			hasSubmission: false,
		...
		},
		{
			...
			date: "⁠29th August afternoon to evening",
			location: "Technikum",
			hasSubmission: false,
	...
	return (
		<section
			className="py-12 md:py-20 px-4 md:px-6 bg-gradient-to-br from-emerald-900 via-teal-800 to-emerald-900 relative overflow-hidden"
			id="information-page"
		>
			...
			<div className="mx-auto relative z-10 max-w-[75vw]">
				...
				<div className="text-center mb-10 md:mb-16 px-4">
					...
					<p className="max-w-2xl mx-auto text-base md:text-lg text-white/80">
						Join us for three extraordinary events shaping
				...
				<div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
					{events.map((event, index) => (
						...
						>
							...
							<div
								className={`absolute inset-0 bg-gradient-to-r ${event.color} opacity-0 transition-opacity duration-500 mx-auto group-hover:opacity-5`}
							/>

							{/* Animated border gradient */}
							<div
								className={`absolute -inset-0.5 bg-gradient-to-r ${event.color} rounded-3xl opacity-0 blur transition-opacity duration-500 -z-10 group-hover:opacity-100`}
							/>
							...
							<div className="relative flex flex-1 flex-col rounded-3xl bg-white p-6 sm:p-8 lg:p-6 xl:p-8">
								<div className="flex flex-1 flex-col items-start gap-4">
									...
									<div className="flex w-full flex-shrink-0  gap-4 lg:flex-col items-center justify-center">
										<div
											className={`h-14 w-14 rounded-2xl border-2 border-gray-100 bg-white shadow-lg transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3 sm:h-18 sm:w-18`}
										>
											...
											event.submissionLabel && (
												<div className="pt-3">
													<a
														...
														className="relative z-20 mx-auto flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-center font-semibold text-white transition-all hover:bg-emerald-700"
													>
														<Image
															src={websiteIcon}
															alt=""
							...
							<div
								className={`absolute top-0 right-0 w-24 h-24 sm:w-32 sm:h-32 bg-gradient-to-br ${event.color} opacity-5 rounded-bl-full`}
							/>
